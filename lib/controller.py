...
class controller(msc_socket):
    ...
    def is_after_login(self):
        return self.platform == Platform.LUANCHER and self.stage == Stage.AFTER_LOGIN and self.status == Status.LOGIN_COMPLETE 
            ...
            while True:
                try:
                    ...
                    if self.status == Status.ASSET_DOWNLOADING :
                        print(f'Asset Downloading{"."*count}')
                    elif self.status == Status.SIM_DOWNLOADING :
                        print(f'Simulator Downloading{"."*count}')
